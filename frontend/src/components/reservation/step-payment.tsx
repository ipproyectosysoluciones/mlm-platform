  ...
  return (
    <div className="space-y-6 py-2">
      ...
      <div className="text-center">
        <h2 className="text-xl font-bold text-[var(--color-foreground)] mb-1">{t('reservation.selectPayment')}</h2>
        <p className="text-[var(--color-foreground-muted)] text-sm">{t('reservation.selectPaymentDesc')}</p>
      </div>
      ...
      <div className="relative space-y-3">
        ...
        >
          ...
          <div className="flex-1 text-left">
            <span className="font-semibold text-[var(--color-foreground)] group-hover:text-emerald-700 transition-colors">
              {t('reservation.payWithPaypal')}
            </span>
            <p className="text-xs text-[var(--color-foreground-muted)] mt-0.5">{formatPrice(totalPrice, currency)}</p>
          </div>
          {processingMethod === 'paypal' ? (
            <Loader2 className="w-5 h-5 text-emerald-500 animate-spin" />
          ) : (
            <ChevronRight className="w-5 h-5 text-[var(--color-foreground-subtle)] group-hover:text-emerald-500 transition-colors" />
          )}
        </Button>
          ...
          <Button
            ...
            className={cn(
              ...
              hasEnoughWalletBalance
                ...
                : 'border-[var(--color-border-subtle)] bg-[var(--color-secondary)] opacity-60 cursor-not-allowed'
            )}
          >
            ...
            <div className="flex-1 text-left">
              <span
                className={cn(
                  'font-semibold',
                  hasEnoughWalletBalance
                    ? 'text-[var(--color-foreground)] group-hover:text-emerald-700 transition-colors'
            ...
            {hasEnoughWalletBalance ? (
              ...
              <AlertCircle className="w-5 h-5 text-[var(--color-foreground-subtle)]" />
            )}
          </Button>
        )}
        ...
        {isProcessingPayment && (
          ...
          >
            ...
            <p className="text-sm font-medium text-[var(--color-foreground)]">{t('loading.processingPayment')}</p>
          </div>
        )}
      </div>

      {/* Security note */}
      <div className="flex items-center gap-2 justify-center text-xs text-[var(--color-foreground-muted)]">
      ...
      <div className="text-center">
        <Button
          ...
          className="gap-2 text-sm text-[var(--color-foreground-muted)] hover:text-emerald-600"
        >
