 ...
 *   money-out. Consumers (WalletService, webhook controller) must go through
 *   `getPayoutGateway` and never instantiate adapters directly.
 *
...
import { mercadoPagoMoneyOutGateway } from './MercadoPagoMoneyOutGateway.js';
...
} from './MercadoPagoMoneyOutGateway.js';
export type { MercadoPagoPayoutWebhookEvent } from './MercadoPagoMoneyOutGateway.js';
...
export function getPayoutGateway(destination: WithdrawalDestination): PayoutGateway {
  ...
  if (destination.accountId) {
    return mercadoPagoMoneyOutGateway;
  }
  throw new AppError(
    400,
    'INVALID_DESTINATION',
